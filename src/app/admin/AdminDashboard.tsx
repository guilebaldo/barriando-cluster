...
export default function AdminDashboard({
  ...
  function accountRowProps(user: AdminUserRow) {
    const isEditing = editingId === user.id;
    return {
      user,
      health: getAccountHealth(user),
      isEditing,
      loadingId,
      pendingPayment: user.status === "manual_pending",
      pendingLink: hasPendingLinkageRequest(user),
      linkageOutcome:
    ...
    <div className="space-y-6 min-w-0 max-w-full">
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        ...
        {pageUsers.length === 0 ? (
          ...
          <>
            {/* Móvil: filas a lo ancho (sin scroll horizontal). */}
            <div className="md:hidden divide-y divide-slate-100">
              {pageUsers.map((user) => {
                const row = accountRowProps(user);
                return (
                  <AccountMobileCard
                    key={user.id}
                    {...row}
            ...
            <div className="hidden md:block overflow-x-auto overscroll-x-contain">
              <table className="w-full text-xs min-w-[860px]">
                ...
                <tbody>
                  {pageUsers.map((user) => {
                    const row = accountRowProps(user);
                    return <UserRows key={user.id} {...row} editForm={editForm} setEditForm={setEditForm} socioOptions={socioOptions} />;
                  })}
                </tbody>
              </table>
            </div>
...
/** Lista móvil: correo + registro + editar a lo ancho (scroll vertical natural). */
function AccountMobileCard({
  ...
  return (
    <article className="px-3 py-3.5 space-y-2.5">
      <div className="flex items-start gap-2.5 min-w-0">
        <div className="pt-1.5 shrink-0">
          <HealthDot status={health} />
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-medium text-slate-900 break-all leading-snug">{user.email}</p>
        ...
        <button
          ...
          data-cuelume-press=""
          data-cuelume-release=""
          className="shrink-0 inline-flex items-center gap-1 px-3 py-2 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-slate-700"
        >
