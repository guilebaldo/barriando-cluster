...
import { allocateNextSocioId, publishBusinessPresenceOnPayment } from "@/lib/publish-business";
...
import {
  ...
  createSocioInviteToken,
  getInviteUrlForUser,
} from "@/lib/socio-invite";
import { getHitoIntro } from "@/lib/map-hito-intro";
import { parseMapCsvFile } from "@/lib/mapRoute";
...
// ─── Alta manual de socio (invitación) ───────────────────────────────────────

const createManualSocioSchema = adminBusinessProfileSchema
  .omit({ socioId: true })
  .extend({
    email: z.string().trim().email("Correo inválido").max(160),
    plan: z.enum(["NEGOCIO_FAMILIAR", "MEDIANA_EMPRESA", "GRAN_EMPRESA"]),
    paymentMethod: z.enum(["stripe", "transfer", "cash", "oxxo"]).nullable().optional(),
...
export async function createManualSocio(
  input: z.infer<typeof createManualSocioSchema>
): Promise<
  | { ok: true; socioId: number; userId: string; inviteUrl: string }
...
> {
  try {
    ...
    const parsed = createManualSocioSchema.safeParse(input);
    if (!parsed.success) {
      return { ok: false, error: parsed.error.issues[0]?.message ?? "Datos inválidos." };
    }
    ...
    const periodEnd = advanceBillingAnniversary(null);
    const formPayload: SocioProfileFormInitial = {
      ...emptyBusinessProfile(email),
      ...data,
      businessName,
      contactEmail: data.contactEmail?.trim() || email,
      privacyAccepted: true,
    };
    const dbFields = toSocioProfileDbFields(formPayload);
    const { privacyAcceptedAt, ...profilePayload } = dbFields;
    const nombre =
      [data.contactFirstName, data.contactLastNamePaternal].filter(Boolean).join(" ").trim() ||
      businessName;
    ...
    await prisma.catalogMembership.create({
      data: {
        socioId,
        plan: data.plan,
    ...
    if (data.latitude != null && data.longitude != null) {
      await prisma.business.upsert({
        ...
        create: {
          ...
          category: data.category.trim() || null,
          website: website || null,
          mapsUrl: data.googleBusinessUrl?.trim() || null,
    ...
    const token = await createSocioInviteToken(user.id);
    const inviteUrl = buildInviteVerifyUrl(appOrigin(), token);

    revalidatePath("/admin");
    return { ok: true, socioId, userId: user.id, inviteUrl };
  } catch (error) {
    if (error instanceof Error && error.message === "UNAUTHORIZED") {
      return { ok: false, error: "Debes iniciar sesión." };
    }
    console.error("[admin] createManualSocio failed:", error);
    return { ok: false, error: "No se pudo crear el socio." };
  }
}

export async function regenerateSocioInviteLink(
  socioId: number
  ...
  try {
    ...
    if (!user) return { ok: false, error: "No hay cuenta vinculada a este socio." };
    if (user.emailVerified) {
      return { ok: false, error: "Este correo ya está verificado." };
    }

    const inviteUrl = await getInviteUrlForUser(user.id, appOrigin());
    if (!inviteUrl) return { ok: false, error: "No se pudo generar el enlace." };
    return { ok: true, inviteUrl };
  } catch (error) {
    if (error instanceof Error && error.message === "UNAUTHORIZED") {
      return { ok: false, error: "Debes iniciar sesión." };
    }
    console.error("[admin] regenerateSocioInviteLink failed:", error);
...
export async function listMapMilestones(): Promise<MapMilestoneRow[]> {
  try {
    const session = await requireSession();
    if (!isAdminUser(session)) return [];
    const rows = await prisma.mapMilestone.findMany({ orderBy: { name: "asc" } });
    return rows.map((r) => ({
      id: r.id,
      name: r.name,
...
const milestoneSchema = z.object({
  ...
  mapsUrl: z.string().trim().url("URL de Maps inválida").max(1000),
  latitude: z.number().min(-90).max(90),
  longitude: z.number().min(-180).max(180),
  zone: z.number().int().min(1).max(20).nullable().optional(),
  active: z.boolean().optional(),
...
export async function createMapMilestone(
  ...
  try {
    ...
    const d = parsed.data;
    await prisma.mapMilestone.create({
      data: {
        name: d.name,
    ...
    return { ok: false, error: "No se pudo crear el hito (¿nombre duplicado?)." };
  }
}
...
export async function toggleMapMilestoneActive(id: string): Promise<ActionResult> {
  try {
    const session = await requireSession();
    if (!isAdminUser(session)) return { ok: false, error: "No autorizado." };
    const row = await prisma.mapMilestone.findUnique({ where: { id } });
    if (!row) return { ok: false, error: "Hito no encontrado." };
    await prisma.mapMilestone.update({
      where: { id },
      data: { active: !row.active },
    });
    ...
    console.error("[admin] toggleMapMilestoneActive failed:", error);
    return { ok: false, error: "No se pudo cambiar el estado." };
  }
}

/** Importa CSV histórico si la tabla está vacía o fuerza upsert por nombre. */
...
> {
  try {
    ...
    const rows = parseMapCsvFile(csvPath);
    let imported = 0;
