    ...
    <div className="space-y-8">
      <section className="grid lg:grid-cols-5 gap-6 items-start">
        <div className="lg:col-span-3 space-y-4">
          <MuaapRouteMap points={route.points} />
        ...
        <aside className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
          ...
          <ol className="space-y-3 max-h-[min(60vh,420px)] overflow-y-auto pr-1">
            {route.points.map((point, idx) => (
              <li
                key={point.id}
                className="flex gap-3 p-3 rounded-xl border border-slate-100 hover:border-slate-200 hover:bg-slate-50/80 transition"
              >
                <span className="shrink-0 w-7 h-7 rounded-full bg-[#27366D] text-amber-400 text-xs font-black flex items-center justify-center">
                  {point.order}
                </span>
